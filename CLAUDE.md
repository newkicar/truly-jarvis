...
## 项目状态
- 当前为**设计阶段，尚未写任何代码**。权威文档：`docs/specs/2026-08-15-javis-design.md`，实现前必须通读。
- 愿景与需求见 `README.md`（注意其中的两条**强制要求**）。
...
## 关键技术决策（实现时容易跑偏）
- 主库 `deepagents`（`create_deep_agent`）；模型走 OpenAI 兼容端点 `opencode.ai/zen/v1`，模型 `deepseek-v4-flash`。
- 交互形态：纯 CLI。
...
- 全局配置用 `javis.json`（模拟 opencode）；可变项放这里，不写死。
...
## 环境与配置陷阱
- `.env` 当前格式不规范（`:` 分隔、小写键）。使用前统一为 `KEY=VALUE`：`BASE_URL` / `API_KEY` / `MODEL_ID` / `TAVILY_KEY`；并把 `.env`、`checkpoints.sqlite` 加入 `.gitignore`。
- 环境：Windows，路径用 `pathlib` 或反斜杠。

## 分期
- 一期：主代理 + researcher（指定检索）+ WIKI 导航知识库 + SqliteSaver 短期记忆 + 会话回退 + javis.json + Tavily。
